...
├── build/                       # Generated artifacts (@tdpl/openapi, @tdpl/validators, @tdpl/enums, @tdpl/db-schema, @tdpl/sdk)
├── packages/
...
**`build/`** -- Generated artifacts (gitignored). Contains `@tdpl/openapi`, `@tdpl/validators`, `@tdpl/enums`, `@tdpl/db-schema`, and `@tdpl/sdk` as file-based packages. Consumed by `server`, `worker`, `client`, and `db` via `file:build/...` dependencies.
...
typespec/
  │ generates
  ▼
build/ (@tdpl/openapi, @tdpl/validators, @tdpl/enums, @tdpl/db-schema, @tdpl/sdk)
  │
  ▼
packages/db/
   │
...
{
  "dependencies": {
    "@tdpl/db-schema": "file:build/db-schema",
    "@tdpl/enums": "file:build/enums",
    "@tdpl/openapi": "file:build/openapi",
    "@tdpl/sdk": "file:build/sdk",
    "@tdpl/validators": "file:build/validators"
  }
}
```
...
| Dev mode stubs                           | StubRuntime + StubCaddy                                        | `@tdpl/runtime` exports `createStubRuntime()` (in-memory VM simulation with auto-incrementing IPs). `@tdpl/caddy` exports `createStubCaddy()` (no-op). Server uses stubs when `NODE_ENV=test` (override with `RUNTIME=tart` for real VMs). |
