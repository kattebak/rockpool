...
Add per-workspace customization via the Dev Containers specification. When a repository contains a `.devcontainer/devcontainer.json`, Rockpool builds a nested Podman container from it and runs code-server inside that container instead of directly in the workspace container. When no devcontainer config is present, behavior is unchanged -- code-server runs in the container as it does today. The workspace container becomes a thin shell (Podman + systemd) and the devcontainer becomes the user's development environment.

## Prerequisites

- [EDD-002](002_MicroVM_Runtime.md) -- container runtime, `configure()` hook
- [EDD-004](004_Web_IDE.md) -- code-server as the IDE, `--abs-proxy-base-path` for path-based routing
- [EDD-005](005_Workspace_Image_Pipeline.md) -- container base image, Debian minimal, no Docker (Podman/Buildah)
- [EDD-011](011_Workspace_Service_Refactor.md) -- workspace-service owns lifecycle, `provisionAndStart()` calls `configure()`
- [EDD-003](003_Caddy_Reverse_Proxy.md) -- Caddy routes workspace traffic to container port 8080
...
| Fall back to current behavior when absent  | Must     | No devcontainer.json = code-server on container host                             |
| Persistent workspace data across restarts  | Must     | Podman named volume survives container rebuilds                           |
| Port forwarding through Caddy              | Must     | Container ports exposed to container host, then routed by Caddy                  |
...
resolveDevcontainer(workspaceFolder)
  ...
  |
  +-- 4. None of the above
         → No devcontainer (current behavior, code-server on container host)
```
...
**Project type detection** is a simple file-existence check via exec. A mapping of marker files to default devcontainer configs:
...
**Repo inspection before create** -- if the workspace creation API accepts a git URL (future), Rockpool could inspect the repo (shallow clone or GitHub API) before container creation to determine whether a devcontainer.json exists and what project type it is. This enables showing the user what will happen ("This repo has a devcontainer.json with Node 20 + PostgreSQL") before committing to the build.

### Two-Path Model

The `configure()` hook in `TartRuntime` (called by `provisionAndStart()` after container boot) resolves the devcontainer config and takes the appropriate path:
...
configure(name, env)
  ...
  |     4. Podman publishes container:8080 -> workspace:8080
  |
  +-- NO: Current path (unchanged)
        1. Write code-server config.yaml
        2. systemctl restart code-server@admin
        3. code-server runs on container host, listening on port 8080
```

### Request Flow (Devcontainer Mode)

```
Browser --> Caddy (srv1) --> container (port 8080) --> Podman container --> code-server
```

Caddy's configuration does not change. It still routes `/workspace/{name}/*` to `CONTAINER_IP:8080`. The difference is that port 8080 in the container is now a Podman-published port forwarding to the container, rather than a process listening directly in the container.

### Request Flow (No Devcontainer, Unchanged)

```
Browser --> Caddy (srv1) --> container (port 8080) --> code-server (on container host)
```
...
```
Browser --> Caddy (srv2) --> workspace (port 3000) --> Podman container --> dev server
```

The `devcontainer up` command publishes ports listed in `forwardPorts` from the container to the container host. For user-registered ports not in `forwardPorts`, additional `podman port` mapping is needed (or we run the container with `--network=host`).

The simplest approach for Phase 1: run the Podman container with `--network=host`. This means all ports the container listens on are directly accessible on the container.s IP. No explicit port mapping is required. This matches the current behavior where code-server and dev servers all listen in the container's network directly.
...
async configure(name: string, env: Record<string, string>): Promise<void> {
    ...
    const containerIp = await getIpForVm(name);
    const hasDevcontainer = await sshExec(containerIp,
        "test -f /home/admin/workspace/.devcontainer/devcontainer.json && echo yes || echo no"
    );

    if (hasDevcontainer.trim() === "yes") {
        await configureWithDevcontainer(containerIp, workspaceName);
    } else {
        await configureWithoutDevcontainer(containerIp, workspaceName);
    }
}
```
...
```typescript
async function configureWithDevcontainer(containerIp: string, workspaceName: string): Promise<void> {
    // 1. Stop the host code-server (we'll run it inside the container instead)
    await sshExec(containerIp, "sudo systemctl stop code-server@admin");
...
- code-server on port 8080 inside the container is accessible at `CONTAINER_IP:8080`
- Dev servers on port 3000 inside the container are accessible at `CONTAINER_IP:3000`
...
The user's code lives at `/home/admin/workspace` on the container. This is bind-mounted into the container by `devcontainer up` (the `--workspace-folder` flag handles this automatically). Since the container volume is persistent across stop/start cycles (per [EDD-005](005_Workspace_Image_Pipeline.md)), the workspace data survives container restarts.
...
Podman named volumes are stored in the container disk and survive container rebuilds. When the user rebuilds their devcontainer (e.g. after changing `devcontainer.json`), their extensions and settings are preserved.

#### Container Image Cache

Podman stores built images in its local store (`~/.local/share/containers/`). Since the container volume persists, built images survive container stop/start cycles. The first `devcontainer up` after cloning is slow (image pull + build); subsequent starts reuse the cached image.

### Phase 5: code-server Inside the Container

#### Why Not Run code-server in the container Host?

code-server on VS Code is architecturally a "remote" connection. The VS Code Remote Extensions protocol does not support chaining (connecting to a remote that itself connects to another remote). If code-server runs in the container and tries to use the Dev Containers extension to connect into the Podman container, it fails because the extension expects to be the first remote hop.
...
| `hostRequirements`                   | No        | Container resources are fixed at creation time         |
...
- **`initializeCommand`** -- Runs on the host machine before container creation. In Rockpool's model, the "host" is the workspace container, not the user's laptop. Semantics are unclear; defer.
- **`hostRequirements`** -- CPU, memory, GPU requirements. Containers have fixed resources at creation; dynamic resizing is not supported.
...
provisionAndStart(id)
  1. Check container status (runtime.status)
  2. Create container if not found (runtime.create + runtime.start)
  3. Start container if stopped (runtime.start)
  4. Get container IP (runtime.getIp)
  ...
  7. Health check (curl CONTAINER_IP:8080)
  8. Add Caddy route
...
The devcontainer.json itself lives in the container filesystem and is tracked in git by the user — it is not stored in the database. The `WorkspaceTemplate` row records the initial template selection, not the current state of the devcontainer config (which the user may have modified).
...
- No `containerStatus` field — whether the devcontainer is running is derived at runtime from the container
...
Let users SSH into the container and configure it themselves.

**Why not:** Exposing SSH breaks the isolation model. The container's SSH would be an internal provisioning mechanism (key-based, no password), not a user-facing service. Exposing it means managing SSH keys, firewall rules, and adding unnecessary complexity. The devcontainer approach gives users a declarative configuration file instead of imperative SSH access.

### Docker Instead of Podman

Use Docker (dockerd) inside the container.
...
- [x] How does Caddy route to a container? **It does not know about containers.** Caddy routes to `CONTAINER_IP:8080`. Whether that port is a host process or a Podman-published port is transparent to Caddy.
- [x] How does workspace data persist? **Container volumes are persistent.** The workspace directory is bind-mounted into the container. Podman named volumes store container-specific state (extensions, settings). Both survive container stop/start cycles.
...
- [x] Do we need to modify the Caddy routing layer? **No.** Port mapping means container ports are accessible via the host. Caddy routes are unchanged.
- [x] What about rootless Podman and port binding? **Host networking sidesteps the issue.** With `--network=host`, the container shares the container.s network namespace. No rootless port-binding restrictions apply.
...
- [ ] What is the disk space budget? Podman images, container layers, and the devcontainer CLI add to the base image size. Need to measure the actual impact on disk.
...
4. Rebuild workspace image
5. Verify: `podman exec` works, `devcontainer --version` works

#### Phase 2: Two-Path `configure()`

1. Add devcontainer detection to `configure()` in `packages/runtime/src/podman-runtime.ts`
2. Implement `configureWithDevcontainer()` using `podman exec` + devcontainer CLI
3. Implement `configureWithoutDevcontainer()` (extract current code)
4. Increase command timeouts for devcontainer builds
...
| Custom | `none` | — | Empty workspace, bring your own setup. Debian · git · curl · make |

Images are pulled dynamically by Podman inside the container on first `devcontainer up`. Nothing is baked into the base image. First build is slower; subsequent starts use the cached image.

**Custom** is today's behavior — no devcontainer, code-server in the container. It is the escape hatch and backward-compatible default.
...
5. In the worker, resolve `template` slug to a `devcontainer.json` and write it to `/home/admin/workspace/.devcontainer/devcontainer.json` via exec before calling `configure()`
...
User preferences sync is implemented in [EDD-020](020_User_Preferences_Sync.md) using DB-backed blobs pushed into containers via exec on workspace start.

When a workspace uses a devcontainer, code-server runs inside the container — not in the container. EDD-020's `writeFile` currently writes to the container host filesystem. For devcontainer workspaces, preferences need to land inside the container instead. Two options:

1. Use `devcontainer exec` to write files into the running container
2. Bind-mount the code-server config directory from the container host into the container, so `writeFile` to the host path is visible inside the container
