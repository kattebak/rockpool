...
3. [x] Enforce workspace caps and concurrency limits from [doc/EDD/007_Data_Model.md](doc/EDD/007_Data_Model.md). Max 999 workspaces, max 3 concurrent starts. Enforced in `workspace-service.create()` and `workspace-service.start()`. Added `countWorkspaces` and `countWorkspacesByStatus` queries to `@rockpool/db`.
